...
void PORTS_init(void) {
    ...
    GPIOC->MODER |=  0x00550000;    /* set pin output mode */
}

void LCD_nibble_write(char data, unsigned char control) {
    /* populate data bits */
    GPIOC->BSRR = 0x0F000000;       /* clear data bits */
    GPIOC->BSRR = data & 0xF00;      /* set data bits */
...
void LCD_data(char data) {
    LCD_nibble_write(data & 0xF00, RS);      /* upper nibble first */
    LCD_nibble_write(data << 8, RS);        /* then lower nibble */
